...
from ue_eval_for_chunk import sentiment_about_ue_evaluation
from remove_disturb import remove_disturb_comment
from comment_chunk import chunker
from noun_merge import merge

def ue_analysis_based_on_comment(filename):
    starttime = datetime.datetime.now()
    print '------BEGIN ue_analysis------' + '\n'

    starttime_step = datetime.datetime.now()
    print '------BEGIN merge nouns------' + '\n'
    readfile = open(filename, 'r')
    comment_merge_file = filename + '_merge'
    writefile = open(comment_merge_file, 'w')
    merge(readfile, writefile)
    writefile.close()
    readfile.close()
    print '------END merge nouns------' + '\n'
    endtime_step = datetime.datetime.now()
    print '------USED TIME merge nouns ', (endtime_step - starttime_step).seconds, 'seconds!------' + '\n' 

    starttime_step = datetime.datetime.now()
    print '------BEGIN chunking------' + '\n'
    readfile = open(comment_merge_file, 'r')
    comment_chunk_file = filename + '_chunk'
    writefile = open(comment_chunk_file, 'w')
    chunker(readfile, writefile)
    writefile.close()
    readfile.close()
    print '------END chunking------' + '\n'
    endtime_step = datetime.datetime.now()
    print '------USED TIME chunking ', (endtime_step - starttime_step).seconds, 'seconds!------' + '\n' 

    starttime_step = datetime.datetime.now()
    print '------BEGIN remove_disturb_comment------' + '\n'
    readfile = open(comment_chunk_file, 'r')
    ...
    word_extend_for_chunk.find_word_about_ue(readfile_inverted_list, readfile_frequency_word, writefile)
    ...
    word_extend_for_chunk.find_word_about_sentiment(readfile_inverted_list, readfile_frequency_word, writefile)
    readfile_inverted_list.close()
    readfile_frequency_word.close()
    writefile.close()
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        exit()
    ue_analysis_based_on_comment(sys.argv[1])